...
public class Thumbnail {
    ...
    private static Bitmap rotateImage(Bitmap bitmap, int orientation) {
        if (orientation != 0) {
            ...
            try {
            	...
            	Log.d(TAG, "Failed to rotate thumbnail" + t);
            }
        }
        return bitmap;
    }
    ...
    public void saveTo(File file) {
        ...
        synchronized (sLock) {
            try {
            	...
            	Log.d(TAG, "Fail to store bitmap. path=" + file.getPath() + e);
            }
            finally {
    ...
    public static Thumbnail loadFrom(File file) {
        ...
        synchronized (sLock) {
            try {
            	...
            	Log.d(TAG, "Fail to load bitmap. " + e);
                return null;
            }
            catch(OutOfMemoryError e) {
            	Log.d(TAG, " fail load outofmemory " + e);
            }
            catch (Exception e) {
            	Log.d(TAG, " fail load bitmap " + e);
                return null;
            }
            finally {
                Util.closeSilently(f);
    ...
    public static Media getLastImageThumbnail(ContentResolver resolver) {
        ...
        String selection = ImageColumns.MIME_TYPE + "='image/jpeg' AND " + ImageColumns.BUCKET_ID + '=' + Storage.BUCKET_ID;
    ...
    private static Media getLastVideoThumbnail(ContentResolver resolver) {
        ...
        Uri query = baseUri.buildUpon().appendQueryParameter("limit", "1").build();
        String[] projection = new String[] {
        VideoColumns._ID, MediaColumns.DATA, VideoColumns.DATE_TAKEN
        };
    ...
    public static Thumbnail createThumbnail(String path, int orientation, int inSampleSize, Uri uri) {
    ...
    private static Thumbnail createThumbnail(Uri uri, Bitmap bitmap, int orientation) {
        if (bitmap == null) {
        	Log.d(TAG, "Failed to create thumbnail from null bitmap");
            return null;
        }
        try {
        	...
        	Log.d(TAG, "Failed to construct thumbnail" + e);
            return null;
        }
    }
}
